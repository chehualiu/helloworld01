...
def getTrendlines(df, extend=30):
    lows = df['isLow'].dropna().tolist()
    low_indexs = df['isLow'].dropna().index.tolist()
    highs = df['isHigh'].dropna().tolist()
    high_indexs = df['isHigh'].dropna().index.tolist()
    trendlines = []
    for i in range(1, len(lows),1):
        if lows[i]>lows[i-1]:
            trendlines.append([(low_indexs[i-1],low_indexs[i]+extend),(lows[i-1],lows[i]+extend/(low_indexs[i]-low_indexs[i-1])*(lows[i]-lows[i-1]))])
    for i in range(1, len(highs), 1):
        if highs[i]<highs[i-1]:
            trendlines.append([(high_indexs[i - 1], high_indexs[i] + extend), (
            highs[i - 1], highs[i]- extend / (high_indexs[i] - high_indexs[i - 1]) * (highs[i-1] - highs[i]))])
...
def dfFilterHL(df):
    ...
    tempitem = None
    dict_new = {}
    for index, data_dict in dict_data.items():
        if tempitem is None:
            tempid, tempitem = index, data_dict
            status = 'checkHigh' if math.isnan(data_dict['isLow']) else 'checkLow'
            continue
        elif status == 'checkHigh' and math.isnan(data_dict['isHigh']):
            dict_new[tempid] = tempitem
            tempid, tempitem = index, data_dict
            status = 'checkLow'
            continue
        elif status == 'checkLow' and math.isnan(data_dict['isLow']):
            dict_new[tempid] = tempitem
            tempid, tempitem = index, data_dict
            status = 'checkHigh'
            continue
        elif status == 'checkHigh' and math.isnan(data_dict['isLow']) and data_dict['isHigh']<tempitem['isHigh']:
            continue
        elif status == 'checkHigh' and math.isnan(data_dict['isLow']) and data_dict['isHigh']>=tempitem['isHigh']:
            tempid, tempitem = index, data_dict
        elif status == 'checkLow' and math.isnan(data_dict['isHigh']) and data_dict['isLow']>tempitem['isLow']:
            continue
        elif status == 'checkLow' and math.isnan(data_dict['isHigh']) and data_dict['isLow']<=tempitem['isLow']:
            tempid, tempitem = index, data_dict

    dict_new[tempid] = tempitem
    modified_df = pd.DataFrame.from_dict(dict_new, orient='index')
    # print("\nModified DataFrame:")
    # print(modified_df)
...
def plotAllzjlx():
    ...
    ax2.text(0.5,0.95,'HS300 主力流入(蓝柱):' + str(round(hs300_boss,2))+' 南向流入(灰柱):' + str(round(south_amt,2)) + ' 上涨数(绿线): '+ str(round(upcnt,0)),
...
def drawAllCCBmin1A():
    ...
    for k,v in etf_dict.items():
        ...
        # df_single.loc[(df_single['extremaup']==True),'up2'] = 0
        # df_single.loc[(df_single['extremadw']==True),'dw2'] = 0
        ...
        ##################  v2.3 add new trend begin  ###########

        if 'high' in df_single.columns:
            useHL = True
        else:
            useHL = False
        lasttime = str(df_single.datetime.values[-1])[5:].replace(' ', '_').replace(':', '').replace('-', '')

        df_single = get_higher_low_up(df_single, col='close', n=hlbars)
        if len(df_single[df_single['isHigh'] == True]) < 6 or len(df_single[df_single['isLow'] == True]) < 6:
            df_single = get_higher_low_up(df_single, col='close', n=hlbars, mode='duplicated')
            print(f'''{k} {v} 使用{'HL' if useHL else 'C'}+等于模式计算高低点  backset:{backset}''')

        df_single['marktmp'] = df_single['extremaup'].map({True: 1, False: 0}) + df_single['extremadw'].map(
            {True: -1, False: 0})
        df_single['marktmp'] = df_single['marktmp'].replace(0, np.nan)
        df_single['marktmp'] = df_single['marktmp'].ffill()
        df_single['markup'] = df_single['marktmp'].replace(-1, np.nan)
        df_single['markdw'] = -1 * df_single['marktmp'].replace(1, np.nan)

        uplist = df_single['markup'].replace(np.nan, 0).tolist()
        if sum(uplist[-5:]) > 0 and sum(uplist[-30:-5]) == 0:
            msg = f'''{k} {v} 向上趋势开始  {lasttime}'''
            print(msg)
            msgURL = pushurl + msg
            requests.get(msgURL)

        # df_single['extremaup'] = df_single.apply(lambda x: df_single.high.values[0] if x.extremaup else np.nan, axis=1)
        df_single['extremaup'] = df_single.apply(lambda x: 0.5 if x.extremaup else np.nan, axis=1)
        df_single['extremadw'] = df_single.apply(lambda x: 0.5 if x.extremadw else np.nan, axis=1)
        df_single['isHigh'] = df_single.apply(lambda x: x.high if x.isHigh else np.nan, axis=1)
        df_single['isLow'] = df_single.apply(lambda x: x.low if x.isLow else np.nan, axis=1)

        df_single['time'] = df_single['datetime'].apply(lambda x: x.split(' ')[1])
        firstbar = df_single['time'].tolist().index('15:00')
        df_trend = df_single[firstbar:]
        if 'index' in df_trend.columns:
            del df_trend['index']
        df_trend.reset_index(drop=True, inplace=True)
        df_trend.reset_index(drop=False, inplace=True)

        temp = df_trend[(df_trend['isLow'].notnull()) | (df_trend['isHigh'].notnull())][['isHigh', 'isLow']]
        temp = temp.loc[(temp['isLow'].shift(-1) != temp['isLow']) & (temp['isHigh'].shift(-1) != temp['isHigh'])]
        temp2 = dfFilterHL(temp)
        trendlines = getTrendlines(temp2, extend=15)
        trendline_dict[k] = trendlines

        del df_trend['isHigh'], df_trend['isLow']
        # del df_trend['isHigh'], df_trend['isLow']
        df_trend = df_trend.merge(temp2, left_index=True, right_index=True, how='left')

        df_trend['HLlines'] = df_trend['isHigh'].fillna(0) + df_trend['isLow'].fillna(0)
        df_HLline = df_trend.loc[df_trend['HLlines'] != 0][['index', 'HLlines']]
        df_HLline_x = df_HLline['index'].tolist()
        df_HLline_y = df_HLline['HLlines'].tolist()

        highlow_dict[k] = [df_HLline_x, df_HLline_y]

        ##################  v2.3 add new trend end  ###########
    ...
    fig, ax = plt.subplots(len(etf_dict), 1, figsize=(18,9), sharex=True)
    ...
    for x,k in zip(ax,  etf_dict.keys()):
        ...
        if len(highlow_dict[k])>0:
            x.plot(highlow_dict[k][0], highlow_dict[k][1], lw=2, color='blue', alpha=0.5,zorder=-20)

        if len(trendline_dict[k])>0:
            for trendline in trendline_dict[k]:
                x.plot(trendline[0], trendline[1], lw=1, color='green', alpha=0.5,zorder=-10)
    ...
    plt.savefig('output\\持续监控ETF1分钟2.3a_' + datetime.datetime.now().strftime('%Y%m%d') + '.png')
...
def get_higher_low_up(df, col='close', n=5, mode='nomal'):
    ...
    if mode=='nomal':
        if useHL:
            df['min'] = df.iloc[argrelextrema(df['low'].values, np.less, order=n)[0]]['low']
            df['max'] = df.iloc[argrelextrema(df['high'].values, np.greater, order=n)[0]]['high']
            df["isHigh"] = (df["high"] == df['max'])
            df["isLow"] = (df["low"] == df['min'])
        else:
            df['min'] = df.iloc[argrelextrema(df[col].values, np.less, order=n)[0]]['close']
            df['max'] = df.iloc[argrelextrema(df[col].values, np.greater, order=n)[0]]['close']
            df["isHigh"] = (df[col] == df['max'])
            df["isLow"] = (df[col] == df['min'])
    else:
        if useHL:
            ...
            df['min'] = df.iloc[argrelextrema(df[col].values, np.less_equal, order=n)[0]]['close']
            df['max'] = df.iloc[argrelextrema(df[col].values, np.greater_equal, order=n)[0]]['close']
            df["isHigh"] = (df[col] == df['max'])
            df["isLow"] = (df[col] == df['min'])


    df.loc[df["isHigh"]==True,'pointstatus'] = 1
    df.loc[df["isLow"]==True,'pointstatus'] = -1
    df['pointstatus'] = df['pointstatus'].fillna(0)
    ...
    if useHL:
        for i in range(0,len(df),1):
            if len(FIFO)<4 and df['pointstatus'].values[i]==0:
                trend.append(0)
            elif len(FIFO)<4 and df['pointstatus'].values[i]==1:
                if len(FIFO)==0:
                    FIFO.append({'flag':1,'close': df['high'].values[i]})
                    trend.append(0)
                else:
                    if FIFO[-1]['flag']==-1:
                        FIFO.append({'flag':1,'close': df['high'].values[i]})
                        ...
                        if df['high'].values[i]>=FIFO[-1]['close']:   # 大于等于前高点，更新为本次高点
                            FIFO.pop()
                            FIFO.append({'flag':1,'close': df['high'].values[i]})
                            trend.append(0)
                        else:
                ...
                if len(FIFO)==0:
                    FIFO.append({'flag':-1,'close': df['low'].values[i]})
                    trend.append(0)
                else:
                    if FIFO[-1]['flag']==1:
                        FIFO.append({'flag':-1,'close': df['low'].values[i]})
                        ...
                        if df['low'].values[i]<=FIFO[-1]['close']:   # 小于等于前低点，更新为本次低点
                            FIFO.pop()
                            FIFO.append({'flag':-1,'close': df['low'].values[i]})
                            trend.append(0)
                        else:
                            trend.append(0)
            elif df['pointstatus'].values[i]==0:   # len(FIFO)>=4
                if FIFO[-1]['flag']==1 :    # 最近极点是高点
                    if FIFO[-1]['close']>FIFO[-3]['close'] and FIFO[-2]['close']>=FIFO[-4]['close']:  # HH&HL
                        if df['close'].values[i]>FIFO[-1]['close']:     # 股价在次高之上
                            trend.append(1)
                        else:   # 股价回落到次高之下
                            trend.append(0)
                    elif FIFO[-1]['close']<=FIFO[-3]['close'] and FIFO[-2]['close']>=FIFO[-4]['close']:  # LH&HL
                        if df['close'].values[i]>max([i['close'] for i in FIFO]): # 股价高过之前的最高价
                            trend.append(1)
                        elif df['close'].values[i]<FIFO[-2]['close']:     # 股价低过上一个低点，即将形成LH&LL
                            trend.append(-1)
                        else:   # 股价回落到次高之下
                            trend.append(0)
                    elif FIFO[-1]['close']<=FIFO[-3]['close'] and FIFO[-2]['close']<FIFO[-4]['close']:  # LH&LL
                        if df['close'].values[i]>max([i['close'] for i in FIFO]):     # 股价高过之前的最高价
                            trend.append(1)
                        elif df['close'].values[i]<min([i['close'] for i in FIFO]):     # 股价低过之前的最低价
                            trend.append(-1)
                        else:
                            trend.append(0)
                    elif FIFO[-1]['close']>FIFO[-3]['close'] and FIFO[-2]['close']<FIFO[-4]['close']:  # HH&LL
                        if df['close'].values[i]>max([i['close'] for i in FIFO]):     # 股价高过之前的最高价
                            trend.append(1)
                        elif df['close'].values[i]<min([i['close'] for i in FIFO]):     # 股价低过之前的最低价
                            trend.append(-1)
                        ...
                        print('logic leak', FIFO)

                elif FIFO[-1]['flag']==-1 : # 最近极点是低点
                    if FIFO[-1]['close']<FIFO[-3]['close'] and FIFO[-2]['close']<=FIFO[-4]['close']:  # LH&LL
                        if df['close'].values[i]>max([i['close'] for i in FIFO]):     # 股价高过之前的最高价
                            trend.append(1)
                        elif df['close'].values[i]<min([i['close'] for i in FIFO]):     # 股价低过之前的最低价
                            trend.append(-1)
                        ...
                        # if df['close'].values[i]>max([i['close'] for i in FIFO]): # 股价高过之前的最高价
                        if df['close'].values[i]>FIFO[-2]['close']: #    # 股价高过上一个高点，即将形成HH&HL
                            trend.append(1)
                        elif df['close'].values[i]<min([i['close'] for i in FIFO]):     # 股价低过之前的最低价
                            trend.append(-1)
                        else:   # 股价回落到次高之下
                            trend.append(0)
                    elif FIFO[-1]['close']>=FIFO[-3]['close'] and FIFO[-2]['close']>FIFO[-4]['close']:  # HH&HL
                        if df['close'].values[i]>max([i['close'] for i in FIFO]): # 股价高过之前的最高价
                            trend.append(1)
                        else:   # 股价回落到次高之下
                            trend.append(0)
                        ...
                        print('logic leak', FIFO)
            elif df['pointstatus'].values[i]==1:  # len(FIFO)>=4
                if FIFO[-1]['flag']==-1:
                    FIFO.append({'flag':1,'close': df['close'].values[i]})
                    trend.append(0)
                elif FIFO[-1]['flag']==1:
                    if df['close'].values[i]>=FIFO[-1]['close']:
                        FIFO.pop()
                        FIFO.append({'flag':1,'close': df['close'].values[i]})
                        trend.append(0)
                    else:
            ...
            if len(FIFO) < 4 and df['pointstatus'].values[i] == 0:
                ...
                if len(FIFO) == 0:
                    ...
                    if FIFO[-1]['flag'] == -1:
                        ...
                        if df['close'].values[i] >= FIFO[-1]['close']:  # 大于等于前高点，更新为本次高点
                            FIFO.pop()
                            FIFO.append({'flag': 1, 'close': df['close'].values[i]})
                            trend.append(0)
                        else:
                ...
                if len(FIFO) == 0:
                    ...
                    if FIFO[-1]['flag'] == 1:
                        ...
                        if df['close'].values[i] <= FIFO[-1]['close']:  # 小于等于前低点，更新为本次低点
                            FIFO.pop()
                            FIFO.append({'flag': -1, 'close': df['close'].values[i]})
                            trend.append(0)
                        else:
    ...
    df['trend'] = trend
    df['extremaup'] = (df['trend']==1)
    df['extremadw'] = (df['trend']==-1)
...
if __name__ == '__main__':
    ...
    hlbars = int(dict(config.items('option_screen'))['hlbars'])
